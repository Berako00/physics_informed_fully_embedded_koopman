...
class AUTOENCODER(nn.Module):
    def __init__(self, Num_meas, Num_inputs, Num_x_Obsv, Num_x_Neurons,
                 ...
                 Num_hidden_u_encoder, Ts, integrated_input_idxs=None, Num_state_notU_affected = None):
        ...
        self.u_koopman_fixed = nn.Linear(Num_u_Obsv + Num_inputs, Num_state_notU_affected, bias=False)
        with torch.no_grad():
            self.u_koopman_fixed.weight.zero_()
        self.u_koopman_fixed.weight.requires_grad = False

        self.u_koopman_train = nn.Linear(Num_u_Obsv + Num_inputs, Num_x_Obsv + Num_meas - Num_state_notU_affected, bias=False)
        #------------------------
    ...
    def x_Koopman_op(self, x):
        x_fixed = self.x_koopman_fixed(x)
        x_train = self.x_koopman_train(x)
